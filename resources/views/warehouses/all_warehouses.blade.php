...
<div class="container-fluid">
    <div class="d-md-flex d-block align-items-center justify-content-between my-2 page-header-breadcrumb">
        <h4 class="page-title fw-semibold fs-18 mb-0">Kho</h4>
        <div class="ms-md-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0 padding">
                    <li class="breadcrumb-item"><a href="javascript:void(0);">Trang Chủ</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Danh Sách Kho</li>
                </ol>
            </nav>
        </div>
    </div>
...
<form id="warehouses-form" action="{{ route('warehouses.save') }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    <div class="modal fade" id="create-warehouses" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Thêm Kho</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body px-4">
                    <div class="row gy-2">
                        <div class="col-xl-12">
                            <label for="warehouse_code" class="form-label">Mã Kho</label>
                            <input type="text" class="form-control" name="code" required placeholder="Mã kho">
                        </div>
                        <div class="col-xl-12">
                            <label for="warehouse" class="form-label">Tên Kho</label>
                            <input type="text" class="form-control" name="name" required placeholder="Tên kho">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Hủy</button>
                    <button type="submit" class="btn btn-success" id="submit-btn-warehouses">Lưu</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card custom-card">
            ...
            {{-- <div class="card-header d-flex justify-content-between align-items-center" style="grid-gap: 3px">
                ...
                <button class="btn btn-sm btn-success btn-wave waves-light" data-bs-toggle="modal"
                    data-bs-target="#create-warehouses">
            ...
            <div class="card-body">
                <table id="warehouses-table" class="table table-bordered text-nowrap w-100">
                    <div id="buttons-container" class="d-flex justify-content-end gap-2">
                        ...
                        <button id="add-selected-btn" class="btn btn-success" data-bs-toggle="modal"
                            data-bs-target="#create-warehouses" style="border-radius: 7px;">
                    ...
                    <thead>
                        <tr>
                            <th></th>
                            <th>
                                <input class="form-check-input check-all" type="checkbox" id="select-all-warehouses"
                                    value="" aria-label="...">
                            </th>
                            {{-- <th scope="col">STT</th> --}}
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Bạn chắc chắn muốn xóa kho đã chọn?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
...
<script>
    $(document).ready(function() {
            ...
            $('#select-all-warehouses').on('change', function() {
                var checked = $(this).prop('checked');
                $('#warehouses-table tbody .form-check-input').each(function() {
            ...
            $('#warehouses-table tbody').on('change', '.form-check-input', function() {
                var WareId = $(this).data('id');
                toggleButtons();
            ...
            $('#warehouses-table').on('draw.dt', function() {
                $('#warehouses-table tbody .form-check-input').each(function() {
                    var WareId = $(this).data('id');
            ...
            $('#delete-selected-btn').on('click', function() {
                ...
                $('#confirmDeleteBtn').on('click', function() {
                    $.ajax({
                        url: '/warehouses/delete-multiple',
                        type: 'POST',
                        data: {
                            ids: [...selectedRows]
                        },
            ...
            function toggleButtons() {
                var selected = $('#warehouses-table tbody .form-check-input:checked').length;
                if (selected > 0) {
...
<script>
    $(document).on('click', '.toggle-status', function(e) {
            ...
            Swal.fire({
                ...
                text: "Bạn có chắc chắn muốn thay đổi trạng thái của kho này?",
                icon: "warning",
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                Swal.fire({
                                    text: 'Trạng thái của kho đã được cập nhật.',
                                    icon: 'success',
                                    confirmButtonText: 'OK',
                                    timer: 3000
                                });
                                ...
                                Swal.fire({
                                    text: response.message ||
                                        'Không thể thay đổi trạng thái của kho.',
                                    icon: 'error',
                                    confirmButtonText: 'OK',
                                    timer: 3000
                                });
